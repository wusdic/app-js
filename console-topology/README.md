...
- **连接与数据流转**：弧线常亮表示有连接；有数据往来时蓝色彗星沿弧线飞过，并显示方向箭头（双向两端都有）；故障连接红色慢速呼吸；临时连接为虚线，淡入淡出。
...
const mod = createTopologyModule(document.getElementById('topology'), { data, layer: 'all', autoRotate: true, intro: true, onSelect: (b) => {} });
