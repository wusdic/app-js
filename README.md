...
- **分层查看**：默认只看核心业务层，其它两层虚化（节点淡化、标签隐藏、圆盘变暗）；左侧「查看层级」或键盘 1 / 2 / 3 / 0 切换，相机自动对准该层。选中层内部的连线常亮，与其它层的连线较淡，与本层无关的连线几乎不可见。
- **宏观视角**：核心 / 重要 / 一般三层业务圆盘自上而下堆叠。业务系统建模为「六边形底座 + 悬浮旋转的线框多面体」（核心：二十面体 + 轨道弧环；重要：二十面体；一般：八面体），接入终端以环绕底座的"尘埃"密度表示在线数量。
- **连接与数据流转**：连线常亮表示"有连接"；有数据往来时一道辉光沿线飞过（持续有数据约每 2 秒一道），并在有流量期间显示方向箭头（双向连接两端都有）；临时连接无数据一段时间后自动消失；故障连接即便无数据也保持可见并闪烁。悬停任一业务或连接会"聚光"：只保留相关的连接与对端，其余压暗；右侧"实时数据流转"列表按频次列出当前层正在流转的连接（谁 → 谁 · 频次），悬停行即在 3D 中高亮，点击可固定。
- **局部视角**：点击任一业务（节点或名称标签均可），相机推近并展开以该业务为中心的"舞台"：内环为组件（含组件间调用关系），外环为本地终端，边缘"出口"示意与其他业务 / 远端终端的连接（远端业务不在本视角展开，点击出口可跳转）。点击组件弹出「组件状态」卡：实例健康数、时延 / 请求量 / 错误率、CPU / 内存、上下游组件、接入终端数、最近事件。
- **根因**：业务异常由组件根因产生并向上汇总，告警列表与业务信息框都显示"根因 组件名：原因"，定位后组件在舞台上以状态色标出。
...
v.focus('core-1');                  // 进入局部视图；v.focus('core-1', 'core-1-c1') 同时选中组件
v.unfocus();
...
| `LAYOUT.layers.*.view` | 查看该层时的相机距离与仰角 |
| `LAYOUT.defaultLayer` | 页面打开时默认查看的层级 |
| `LAYOUT.camera / focus` | 默认视角、聚焦时相机相对偏移 |
| `LINK_RULES.idleAfter` | 无数据多久后连接熄灭（秒） |
| `LINK_RULES.transientRemoveAfter` | 临时连接无数据多久后移除（秒） |
| `LINK_RULES.pulseGap / pulseSpeed / tail` | 辉光的最小间隔、速度、尾巴长度 |
| `LINK_RULES.base` | 底线亮度：本层内 / 跨层 / 无关 / 查看全部 |
