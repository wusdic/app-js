...
- **分层查看**：默认只看核心业务层，其它两层虚化（节点淡化、标签隐藏、圆盘只剩轮廓）；左侧「查看层级」、键盘 1 / 2 / 3 / 0 或点击层级 KPI 切换，相机自动对准该层。当前层内部的连线常亮，跨层连线较淡，无关连线几乎不可见；其它层若有故障，层选择器下方会提示并可一键切过去。
...
| `LAYOUT.defaultLayer / camera / focus` | 默认层级、查看全部的视角、聚焦时的相机偏移与视场角 |
