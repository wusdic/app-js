...
- **宏观视角**：核心 / 重要 / 一般三层业务圆盘自上而下堆叠，业务节点发光，接入终端以环绕的"尘埃"密度表示在线数量。
- **连接与流光**：业务间有数据往来时，连线上自动产生流光；持续有数据则每 1.6～2.5 秒一次；一段时间无数据连线变暗，临时连接自动消失。
...
src/
  ...
  scene/BusinessNode.js 业务节点（球体 + 光晕 + 标签 + 警示环）
  scene/FlowTube.js    带流光的管线（着色器实现彗尾）
...
// 连接上产生一次数据往来（dir: 1 = from→to，-1 = 反向）。持续调用即持续流光；停止调用后连线自动变暗
v.touchLink('l12', 1);
...
| `LINK_RULES.idleAfter` | 无数据多久后连线变暗（秒） |
| `LINK_RULES.transientRemoveAfter` | 临时连接无数据多久后移除（秒） |
| `LINK_RULES.minPulseGap` | 同一连接两次流光的最小间隔（秒） |
